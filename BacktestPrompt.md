...
Готово. Добавил GET /api/ai/overview в backend/main.py — один открытый эндпоинт без авторизации, отдающий сразу всё, что раньше приходилось собирать по HTML-страницам вручную:
...
Ничего не выдумывал заново — эндпоинт просто переиспользует уже существующие функции (get_db_stats, get_bet_type_stats, get_roi_stats, get_bankroll_state и уже готовые прокси к ai_service), просто собирает их в один ответ вместо пяти отдельных запросов.

Параметры (все опциональные, с разумными дефолтами):

bet_types_limit=5 — сколько типов ставок на спорт (полная разбивка может быть сотнями строк, как было видно в вашей выгрузке); 0 — только итоги по спорту
logs_limit=60, backtest_runs=10

Пример: GET https://necrolich.ru/api/ai/overview?bet_types_limit=10