...
7. **Neural verdict = EV for staking, outcome call for stats**: the PyTorch GRU still has a 4-logit head for checkpoint compatibility, but live bankroll bets only consider outcomes where `will_win=1` **and** live gates pass. The «Выигрывающие» tab is `will_win=1` including «выиграет · не ставить». Money band is **1.1–2.0**, plus **2.0–2.5 only when calibrated p ≥ 90%** (`NEURALBET_MAX_BET_COEFF_HIGH_P` / `NEURALBET_HIGH_P_STAKE`). Stake also requires calibrated p ≥ **70%** (`NEURALBET_MIN_STAKE_P`). Training / decision-head masks stay at `MAX_BET_COEFF=2.0`. 1X2 uses a live-score prior (at 1:0 П1 is `will_win`, X/П2 are not). Outside the 1.1–2.0 money band we **analyze** (skip, will_win, or «скорее всего не победит») — never auto-record a miss. Falling live coeff nudges calibrated p up; rising coeff nudges it down (`NEURALBET_COEFF_MOVE_P_WEIGHT`). Residual `decision_logit` training is off by default (`NEURALBET_DECISION_LOSS_WEIGHT=0`). Stake path: will_win → EV → live gates → quality gate → Kelly (no LLM). Overall `accuracy_pct` ≈ 50% is expected on 2-way lines and is not the edge KPI — use walk-forward ROI CI and Brier vs market. History guessed / not guessed uses **`will_win` vs `is_win`** (not raw EV-skip `predicted_win`). Stake ⊂ will_win.
...
| Tool | What it returns |
| :--- | :--- |
| `get_top_neurobets` | Active LIVE predictions (`verdict=win` = `will_win=1`, including «выиграет · не ставить»; optional `market` = result/totals/handicap/itotal/other; bot stake still requires live gates) |
