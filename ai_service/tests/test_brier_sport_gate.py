...
def _row(sport: str, brier: float, market: float, evaluated: int = 5000) -> dict:
    return {
        "sport": sport,
        "evaluated": evaluated,
        "brier": brier,
...
class BrierSportGateTests(unittest.TestCase):
    def test_select_requires_clear_margin(self):
        selected, detail = select_brier_stake_sports(
            [
                _row("Футбол", 0.1582, 0.1797),
                _row("Настольный теннис", 0.2020, 0.2033),
                _row("Баскетбол", 0.2028, 0.2385),
                _row("Теннис", 0.1746, 0.1740),
                _row("Волейбол", 0.1988, 0.1990),
            ],
            min_evaluated=2000,
            margin=0.005,
        )
        self.assertEqual(selected, ["баскетбол", "футбол"])
        enabled = {d["sport"]: d["enabled"] for d in detail}
        self.assertTrue(enabled["футбол"])
        self.assertTrue(enabled["баскетбол"])
    ...
    def test_select_skips_thin_sports(self):
        selected, _ = select_brier_stake_sports(
            [_row("Теннис", 0.10, 0.20, evaluated=100)],
            min_evaluated=2000,
            margin=0.005,
        )
